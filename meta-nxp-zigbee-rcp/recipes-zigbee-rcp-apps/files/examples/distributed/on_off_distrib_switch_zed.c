...
/* Short address of parent. Will be set after f&b */
zb_uint16_t g_dst_short_address = 0xffffu;
...
MAIN()
{
  ...
  ZB_INIT("on_off_distrib_switch_zed");
  ...
  zb_enable_joining_to_distributed(ZB_TRUE);
                                    ...
                                    zb_uint16_t cluster)
{
  ...
  g_dst_short_address = zb_address_short_by_ieee(addr);
...
void send_toggle_req(zb_uint8_t param)
{
  zb_uint16_t addr = g_dst_short_address;
...
static void active_ep_callback(zb_bufid_t buf)
{
  ...
  if (resp->status != ZB_ZDP_STATUS_SUCCESS || resp->nwk_addr != g_dst_short_address)
  {
...
static void send_active_ep_req(zb_bufid_t buf)
{
  ...
  else
  {
    ...
    req->nwk_addr = g_dst_short_address;
...
static void simple_desc_callback(zb_bufid_t buf)
{
  ...
  if (resp->hdr.status != ZB_ZDP_STATUS_SUCCESS || resp->hdr.nwk_addr != g_dst_short_address)
  {
...
void button_press_handler(zb_uint8_t param)
{
  ...
  {
    WCS_TRACE_NOTICE("button_press_handler() param %d", param);
